...
function createShader(source, typ)
    ...
    if !validateShader(shader)
        error("Shader creation error: ", getInfoLog(shader))
    end
    shader
end
function createShaderFromFile(filepath, typ)
    const source = read_interpolated_string(filepath)
    createShader(source, typ)
end
